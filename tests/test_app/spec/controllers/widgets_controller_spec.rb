...
describe WidgetsController, type: :controller  do
  ...
  it "should not log impression when user-agent is in wildcard list" do
    allow_any_instance_of(ActionDispatch::Request).to receive(:user_agent).and_return('somebot')
    get "show", params: { id: 1 }
    expect(Impression.all.size).to eq 11
  end

  it "should not log impression when user-agent is in the bot list" do
    allow_any_instance_of(ActionDispatch::Request).to receive(:user_agent).and_return('Acoon Robot v1.50.001')
    get "show", params: { id: 1 }
    expect(Impression.all.size).to eq 11
  end
